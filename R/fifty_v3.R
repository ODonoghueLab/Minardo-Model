...
calc50crossing_v3 <- function(timeRegions, clustered, phosTh=0.5, dephosTh=0.5){
	...
	for (clusNum in 1:nrow(clustered$center)){
		for (i in 1:nrow(timeRegions[[clusNum]])){
			...
			# print(paste(".... ", startTp, endTp, crossPt[1], crossPt[2], timeRegions[[clusNum]][i, cols_timeReg$dir]))
			if (crossPt[1] == -1){
				# print(clustered$center[clusNum, startTp:endTp])
			}
			# print(crossPt[1])
...
calcCrossing_v3 <- function(region, dir, offset, phosTh, dephosTh){
	...
	# print(paste("Value here is: ", y_min, y_max, y_50, x_50))
...
getTimeRegionsWithMaximalChange <- function(glmTukeyForEachClus, numTps, signifTh=0.05, phosZscoreTh=0, dephosZscoreTh=0, asPercent=TRUE){
	...
	for (i in 1:length(glmTukeyForEachClus)){ # for each cluster
		...
		# print(timeRegions[nrow(timeRegions),])
